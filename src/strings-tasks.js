...
function removeFirstOccurrences(str, value) {
  ...
  const result = str.slice(0, first) + str.slice(first + value.length);
  return result;
}
...
function removeLastOccurrences(str, value) {
  ...
  const result = str.slice(0, last) + str.slice(last + value.length);
  return result;
}
...
function formatTime(minutes, seconds) {
  ...
  let secondsTime = '';
  if (minutes < 10) {
    minutesTime = String(minutes).padStart(2, '0');
  } else {
    minutesTime = String(minutes);
  }
  if (seconds < 10) {
    secondsTime = String(seconds).padStart(2, '0');
  } else {
    secondsTime = String(seconds);
  }
  return minutesTime.concat(':', secondsTime);
}
...
function countVowels(str) {
  ...
  const splitted = str.split('');
  return splitted.filter((item) => vowels.includes(item)).length;
}
...
function isPalindrome(str) {
  const schema = /[\W_]/g;
  const low = str.toLowerCase().replace(schema, '');
  const reverse = low.split('').reverse().join('');
  return reverse === low;
}
...
function findLongestWord(sentence) {
  ...
  const sorted = splitted.sort((a, b) => {
...
function invertCase(str) {
  ...
  for (let i = 0; i < splitted.length; i += 1) {
    ...
    for (let x = 0; x < item.length; x += 1) {
      if (item[x].toUpperCase() === item[x]) item[x] = item[x].toLowerCase();
      else {
        item[x] = item[x].toUpperCase();
      }
    }
    splitted[i] = item.join('');
  }
  return splitted.join(' ');
}
...
function encodeToRot13(str) {
  ...
  return str.replace(/[a-z]/gi, (letter) => second[first.indexOf(letter)]);
}
